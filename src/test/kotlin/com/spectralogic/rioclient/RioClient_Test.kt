...
class RioClient_Test {
    ...
    fun systemRioClientTest() = blockingTest {
        ...
        var testNum = 0
        val testDesc = "systemRioClientTest %d"
        ...
        try {
            rioClient.getRioClient(rc1.id).let { resp ->
                assertThat(resp.statusCode).describedAs(testDesc.format(++testNum)).isEqualTo(HttpStatusCode.BadRequest.value)
            }
        } catch (e: RioHttpException) {
            assertThat(e.statusCode).describedAs(testDesc.format(++testNum)).isEqualTo(HttpStatusCode.BadRequest)
        }
        rioClient.listRioClientApplications().let { resp ->
